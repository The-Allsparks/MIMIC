...
| Piece tracker | Observe-only `PieceTracker` (capacity, TeamCode identity strings, confidence, entry-vs-count reconcile); unused by session; unknown occupancy when sensors missing or disagree | [#64](https://github.com/The-Allsparks/MIMIC/issues/64) | piece observe | None if observe-only | sensors | 1+ | yes | yes |
