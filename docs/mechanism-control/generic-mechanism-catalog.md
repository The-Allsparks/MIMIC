...
| Faults | Invalid, disagree, unexpected limit, stall, jam, skew, failed home, timeout, unexpected motion, lost calibration, insufficient AMPER grant, latch unknown | Policy table now ([#63](https://github.com/The-Allsparks/MIMIC/issues/63)); observe-only stall/jam ([#60](https://github.com/The-Allsparks/MIMIC/issues/60)); reverse-clear / Phase 8 recovery later ([#20](https://github.com/The-Allsparks/MIMIC/issues/20)) |
| Interlocks | Named `InterlockRule` contracts now (reject / defer / clamp / intermediate / confirm tables; no scheduler). Engine that commands hardware is Phase 7 / [#19](https://github.com/The-Allsparks/MIMIC/issues/19) | contracts now ([#62](https://github.com/The-Allsparks/MIMIC/issues/62)); engine 7 |
| Piece tracking | Observe-only presence/count (`PieceObservation`) plus optional `PieceTracker` occupancy, TeamCode identity strings, capacity, confidence, and entry-vs-count reconcile. Missing sensor is unknown occupancy, not empty. Disagreement is unknown (no invented count). Reject routing that would run motors stays later | [#54](https://github.com/The-Allsparks/MIMIC/issues/54) observe; [#64](https://github.com/The-Allsparks/MIMIC/issues/64) tracker; no season piece names |
...
### 7.1 Piece presence, count, and tracking (observe-only)
...
Optional `PieceTracker.capacity(3).identitySlot(0, teamProvidedLabel)` ([PieceTracker.java](../../src/main/java/org/allsparks/mimic/observe/PieceTracker.java)) reconciles that observation into path occupancy, integer count, TeamCode identity strings, and confidence. Zero sensors stay unknown occupancy, not empty. Entry present vs count `0`, or entry absent vs count greater than `0`, is disagreement: occupancy becomes unknown and the tracker does not invent a count. Identity labels are TeamCode-provided strings, not season enums. Unknown identity is first-class when occupancy is unknown or a count cannot uniquely fill pockets. `MimicSession` does not construct a tracker. No auto-spit, no motor writes, no ViDAR ownership. Debounce lives elsewhere. Reject routing that would run motors stays later.

This is not actuation. Core Java must not name season pieces.
...
### 7.5 Interlock contracts (table evaluation only)

`InterlockRule.when("feeder").requires("launcher", READY).onFail(REJECT)` ([InterlockRule.java](../../src/main/java/org/allsparks/mimic/config/InterlockRule.java)) is a named constraint, not a scheduler. `evaluate(InterlockInputs)` returns `GoalResult` with existing `GoalDisposition` values. `READY` may be a named state or a `Readiness.ready()` bit copied into the table. Fail outcomes: reject / defer / clamp / intermediate / confirm. Confirm maps to `DEFERRED` with reason `INTERLOCK_CONFIRM_REQUIRED`; it does not add `CONFIRM` to `GoalDisposition`. Generated intermediates must not cycle. `MimicSession` does not register rules. `permitsMotion()` is false. The engine that would command hardware remains [#19](https://github.com/The-Allsparks/MIMIC/issues/19). See [interlocks.md](interlocks.md).

### 7.6 Fault and degraded-mode policy (table only)

`FaultPolicy.severity(FaultKind.SENSOR_DISAGREEMENT, DegradedBehavior.STOP_MECHANISM)` ([FaultPolicy.java](../../src/main/java/org/allsparks/mimic/config/FaultPolicy.java)) maps a catalog fault kind plus the per-role `DegradedBehavior` onto `INFO` / `DEGRADED` / `STOP_MECHANISM` / `STOP_DEPENDENCIES` / `STOP_ROBOT`. Each catalog kind has a default. Optional-sensor invalid/disagree may be `INFO` when the role is `IGNORE_OPTIONAL`. Latch unknown never auto-releases and floors at `STOP_MECHANISM`. Required sensors still cannot `IGNORE_OPTIONAL` (`ConfigurationValidator`). `MimicSession` does not consult the table; `status()` stays `DEGRADED` on an invalid snapshot only. `permitsMotion()` and `permitsRecoveryMotion()` are false. Do not enable `MimicFeatureFlags.phase8Faults`. Recovery motion stays [#20](https://github.com/The-Allsparks/MIMIC/issues/20). AMPER grant faults that change output stay [#21](https://github.com/The-Allsparks/MIMIC/issues/21). See [fault-handling.md](fault-handling.md).
...
Allowed now: families, constructs, topology, sensor roles, capabilities, immutable configuration, named-state name sets, snapshot role extras, observe-only piece presence/count (`PieceObservation`), optional observe-only `PieceTracker` (unused by session), pure `Readiness` at-speed / in-tolerance evaluation, observe-only stall/jam suspicion (`StallDetector` / `JamDetector`), optional unused `SyncContract` on independently sensed topology, named `InterlockRule` contracts with table evaluation (unused by session), `FaultPolicy` severity lookup (unused by session; no recovery motion), declaration-only `CalibrationContract` / `LimitContract` / `MechanismControllerAdapter` (unused by session), richer presets, family observation sketches, validation, tests, this document, [gap matrix](generic-mechanism-gap-matrix.md). Catalog work is stacked on [PR #71](https://github.com/The-Allsparks/MIMIC/pull/71): [#49](https://github.com/The-Allsparks/MIMIC/issues/49)–[#54](https://github.com/The-Allsparks/MIMIC/issues/54), [#56](https://github.com/The-Allsparks/MIMIC/issues/56)–[#65](https://github.com/The-Allsparks/MIMIC/issues/65). Debounce stays on [PR #73](https://github.com/The-Allsparks/MIMIC/pull/73) / [#55](https://github.com/The-Allsparks/MIMIC/issues/55).

Forbidden now: motor/servo writes, homing motion, limit enforcement that writes, controller output reaching hardware, interlock engines that command, reject routing that writes motors, Phase 2–10 flags. Hardware observation, acceptance sign-off, and active control remain [#6](https://github.com/The-Allsparks/MIMIC/issues/6), [#68](https://github.com/The-Allsparks/MIMIC/issues/68), [#69](https://github.com/The-Allsparks/MIMIC/issues/69). Per-family active implementations remain [#70](https://github.com/The-Allsparks/MIMIC/issues/70).

Java: `org.allsparks.mimic.templates` (families/constructs/blueprints), `org.allsparks.mimic.config` (configuration, `InterlockRule`, `FaultPolicy`), `org.allsparks.mimic.control` (unused adapter), and `org.allsparks.mimic.observe` (snapshot extras, `PieceObservation`, `PieceTracker`, `Readiness`, `StallDetector`).
