...
Java: `org.allsparks.mimic.templates` (families/constructs/blueprints), `org.allsparks.mimic.config` (configuration, `InterlockRule`, `FaultPolicy`), and `org.allsparks.mimic.observe` (snapshot extras, `PieceObservation`, `Readiness`, `StallDetector`).
